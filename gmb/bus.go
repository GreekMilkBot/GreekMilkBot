...
func NewClientBus(botID string, call ClientCallHandle) *ClientBus {
	return &ClientBus{
		id:   botID,
		call: call,
	}
...
func (b *ClientBus) SendMessage(receive *bot.Message, message *bot.ClientMessage) (string, error) {
...
func (b *ClientBus) SendPrivateMessage(userID string, message *bot.ClientMessage) (string, error) {
	var messageId string
	return messageId, b.call(b.id, "send_private_msg", []any{userID, message}, []any{&messageId}, 1*time.Second)
}

func (b *ClientBus) SendGroupMessage(groupId string, message *bot.ClientMessage) (string, error) {
	var messageId string
